...
find_interf <- function(data) {
  ...
  df <- data %>%
    mutate(interval = day_sec - lag(day_sec, default = first(day_sec) - 1),
           d_h2o2 = (h2o2_conc - lag(h2o2_conc, n = 1, default = first(h2o2_conc))) / interval,
           d_d_h2o2 = abs(d_h2o2 - lag(d_h2o2, n = 1, default = first(d_h2o2))))

  # Identify interferences and shift by one second
  df <- df %>%
    mutate(int = if_else(d_d_h2o2 > 5, 1, 0),
           int = if_else(lead(int) == 1, 1, 0)
          #,int = if_else(lead(int) == 1, 1, int) # remove and shift only by one second
           )

  # Interference also all data after actual interference,
  # until values goes back to +5 from the median of the 10 seconds before
  df <- df %>%
    mutate(lag_median = apply(sapply(1:10, lag, x = h2o2_conc), 1, median, na.rm = TRUE),
           ref_h2o2 = if_else(int == 1, lag_median, NULL)) %>%
    tidyr::fill(ref_h2o2) %>%
    mutate(int2 = case_when(h2o2_conc > (ref_h2o2 + 5) ~ 1,
                            int == 1 ~ 1,
                            TRUE ~ 0)) %>%
...
data1 <- lapply(data, find_interf)
data1 <- lapply(data1, function(x)
  x %>%
    filter(interference == 0) %>%
    select(-interference))
data2 <- lapply(data1, find_interf)
...
plots <- lapply(1:length(data), function(x) {
  df <- data[[x]]
  df1 <- data1[[x]]
  df2 <- data2[[x]]
...
lapply(1:length(data2), function(x)
  readr::write_csv(data2[[x]],
                   here("data", "cleaned_data", "h2o2_no_interferences", paste(files[x], "clean.csv", sep = "_")))
  )

rm(data, data1, data2, files, find_interf, plots)
