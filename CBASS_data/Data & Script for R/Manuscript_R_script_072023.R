...
# load data set
# baseline temperature control: PAM measurements MAREE & CBASS1 27°C and 30°C
...
VBA_cbass8 <- read.table("CBASS8_VBA.csv", header = T, sep=";") 

# transform data
PAM_control$Fac.Temperature <- as.factor(PAM_control$Temperature)
...
# GLS model
pam_control_gls <- gls(Pam.mean ~ Fac.Temperature, data = PAM_control)
...
matplot(temp_x, predict(PAM_cbass1_DRC, data.frame(Temp = temp_x), interval="confidence"),
        ...
        with(PAM_cbass1,matpoints(Temperature,Pam.mean,pch=18, col="#2c7bb6", cex=1.5))
        legend("bottomleft","P. damicornis",pch=18, col="#2c7bb6",pt.cex=1.5, bty="n",cex=1)+
        title(main="Photosynthesic efficiency")+
        abline(v=ED50_PAM_cbass1, col=c("#2c7bb6"))
        text(paste(ED50_PAM_cbass1), c(0.1),labels=paste(as.character(round(ED50_PAM_cbass1, digits=1.5)),"°C",sep=""),col=c("#2c7bb6"), cex=1.5)
...
matplot(temp_x, predict(PAM_cbass8_DRC, data.frame(Temp = temp_x), interval="confidence"),
        ...
        with(PAM_cbass8,matpoints(Temperature,Pam.mean,pch=18, col="#2c7bb6", cex=1.5))
        legend("bottomleft","P. damicornis",pch=18, col="#2c7bb6",pt.cex=1.5, bty="n",cex=1)
        title(main="Photosynthesic efficiency")
